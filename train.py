...
from utils import MyLoader, init_seed, generate_kg_batch, resolve_device
...
def parse_args():
    ...
    parser.add_argument("--device", default='auto', help="Device: auto, cuda:0, mps, or cpu")
...
def main():
    ...
    config['device'] = resolve_device(args.device, args.gpu_id)
    seed = args.seed
    Ks = config['Ks']
    init_seed(seed, True)
    print('-'*100)
    ...
    num_nodes = config['users'] + config['entities']
    ...
    graph_ig = Data(edge_index=edge_index_ig.contiguous(), num_nodes=num_nodes)
    ...
    graph_kg = Data(edge_index=edge_index_kg.contiguous(), num_nodes=num_nodes)
    graph_kg = graph_kg.to(config['device'])
    print(f'Is kg no duplicate edge: {graph_kg.is_coalesced()}')

    edge_index = torch.LongTensor(edge_index)
    edge_index = coalesce(edge_index)
    graph = Data(edge_index=edge_index.contiguous(), num_nodes=num_nodes)
    ...
    graph_cf = Data(edge_index=edge_index_cf.contiguous(), num_nodes=num_nodes)
